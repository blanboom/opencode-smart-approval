...
import { DEFAULT_REVIEW_CONNECTION, DEFAULT_RISK_TOOL, defaultConfigJson, defaultPolicy } from "./default-config";
...
const reviewFromUnknown = (value: unknown): ReviewConfig => {
  ...
  return { baseURL, apiKey, model, timeoutMs, maxScriptBytes, maxToolCalls, contextMessages, prompt };
};
...
const policyFromUnknown = (value: unknown, fallbackRules: readonly CommandRule[]): ApprovalPolicy => {
...
export const loadOrInitializePolicy = (directory: string): PolicyLoadResult => {
  const path = join(directory, POLICY_FILE_NAME);
  const legacyPath = join(directory, LEGACY_POLICY_FILE_NAME);
  const loadPath = existsSync(path) || !existsSync(legacyPath) ? path : legacyPath;
  const fallbackRules = defaultPolicy().rules;
  if (!existsSync(path)) {
    if (existsSync(legacyPath)) {
      try {
        const policy = policyFromUnknown(parseJsonc(readFileSync(legacyPath, "utf8")), fallbackRules);
        return { ok: true, policy, path: legacyPath, initialized: false };
      } catch (error) {
        const message = error instanceof Error ? error.message : "unknown legacy policy load failure";
        return {
          ok: false,
    ...
    try {
      ...
      return { ok: true, policy: defaultPolicy(), path, initialized: true };
    } catch (error) {
  ...
  try {
    const policy = policyFromUnknown(parseJsonc(readFileSync(loadPath, "utf8")), fallbackRules);
    return { ok: true, policy, path: loadPath, initialized: false };
  } catch (error) {
