...
项目本地配置默认禁用。如需保留旧版的完整替代行为，请在可信的全局文件中设置 `"allow_local_config": true`。项目本地文件不能自行开启这项权限。启用后会作用于你打开的每个项目，因此只能在所有项目及其配置文件都可信时使用。

### 从 v0.2.4 或更早版本升级

旧版本总是优先使用项目本地的 `command-approval.jsonc` 或旧版 `command-approval.json`。从 v0.3.0 开始，这两个文件默认都会被忽略，避免不可信仓库替换你的全局审核器、Tirith 设置或审批规则。

最安全的迁移方式是把可信的项目设置移入全局文件，并删除本地策略。如果你打开的每个项目都可信且确实需要旧行为，可在全局文件中设置 `"allow_local_config": true`；这会把完整策略委托给每个项目，包括审核器凭据、Tirith 行为和 allow 规则。
